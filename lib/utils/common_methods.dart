...
class CommonMethods {
  ...
  Future redeemsLoyaltyPoints({String reward_id_selected}) async {
    String url = '${ApiProvider.BASE_URL}cart/redeemloyalty';
    ...
    final redeemPointsMap = jsonEncode({
      "reward_id_selected": reward_id_selected,
      "orderid": "",
    });
    print("REDEEM POINTS MAP   $redeemPointsMap");

    try {
      final response =
          await http.post(url, body: redeemPointsMap, headers: headers);
      var responseJson = _response(response);
      print("REDEEM POINTS RESPONSE   $responseJson");
    } catch (e) {
      print(e);
    }
  }
  ...
  static getPreference(BuildContext context, String key) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    if (prefs.get(key) != null) return prefs.get(key);
  }
  ...
  static final EdgeInsets textFieldPadding =
      EdgeInsets.only(left: 10.0, right: 10.0);
  static final EdgeInsets initialPadding =
